...
from simulate_tournament_games import init_tournament_worker, play_tournament_game
from Config import config


def discover_checkpoints(networks_dir='Networks', prefix='Episode'):
    # Scans networks_dir for <prefix>_N.keras files - 'Episode' for self-play
    # training (TrainingManager.py), 'Pretrain' for the raw-MCTS bootstrap phase
    # (PretrainManager.py). Prints an episode -> mtime table so a mixed-run
...
def snapshot_checkpoints(selected, snapshot_root='Networks/tournament_snapshots', prefix='Episode'):
...
def build_raw_mcts_specs(depths):
    ...
    # fixed search depths - same depths ELO.py's own baseline calibration uses. These
...
def run_tournament(agent_specs, iterations, num_processes, log_path='tournament_log.csv'):
...
if __name__ == '__main__':
    ...
    ITERATIONS = 192*2
    RAW_MCTS_DEPTHS = [16, 64, 256, 1024, 4096]  # same spread as ELO.py's own baseline calibration

    print(
        "Note: network agents search with whatever Config.py's mcts.search_depth "
        ...
        "use their own fixed depths regardless of Config.py.\n"
    )

    # Two independent ladders, since Episode_N (self-play) and Pretrain_N (raw-MCTS
    # bootstrap - see PretrainManager.py) checkpoints share the numeric episode
