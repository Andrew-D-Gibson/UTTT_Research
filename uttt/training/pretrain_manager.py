...
from uttt.paths import (
    NETWORK_PATH,
    NETWORKS_DIR,
    PRETRAIN_POOL_PATH as POOL_PATH,
    PRETRAIN_LOG_PATH,
    ensure_data_dirs,
)
...
class PretrainManager:
    ...
    # uttt/training/generate_raw_pool.py) and no candidate/champion gating (there's no
    # opponent to gate against here - progress is judged externally, by watching
    # data/logs/pretrain_log.csv and running uttt/evaluation/network_tournament.py). Every round trains
    ...
    def __init__(self):
        ...
        if not os.path.exists(POOL_PATH):
            raise FileNotFoundError(
                f'{POOL_PATH} not found - run uttt/training/generate_raw_pool.py first to build the raw-MCTS example pool.')
        ...
        self.log_path = PRETRAIN_LOG_PATH
        self.log_fields = [
    ...
    def pretrain(self):
        ...
        for round_num in range(num_rounds):
            ...
            # Episode_N. uttt/evaluation/network_tournament.py picks these up alongside Episode_N
            # checkpoints once you switch over to self-play.
            self.network.save(os.path.join(NETWORKS_DIR, f'Pretrain_{round_num}.keras'))
