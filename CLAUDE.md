...
All entry points are thin **launchers at the repo root** (`project.py`, `pretrain.py`,
`elo.py`, `network_tournament.py`, `interface.py`) that import the real logic from the
...
- **Pretrain from raw MCTS** (optional, fire-and-forget alternative starting point to `project.py`):
  ...
  network-free MCTS (`uttt/training/generate_raw_pool.py`, `MCTS(network=None)` at
   `config['mcts']['pretrain_mcts_depth']`, default 4096) up to `training.max_training_examples`
  examples, saved to `data/PretrainExamples/pool.pkl` — a no-op once the pool already meets that target,
  ...
  round is kept unconditionally — checkpointing every round to `data/Networks/Pretrain_N.keras` and
   `data/Network.keras`, logging to `data/logs/pretrain_log.csv` (same columns as `training_log.csv`). This is a
  ...
  stronger no matter how many times its pool is regenerated, so once `pretrain_log.csv`/
   `network_tournament.py` show diminishing returns across pool generations, switch to
   `python project.py`, which resumes from wherever `data/Network.keras` was left.
...
- **Training gates candidates against the champion; it doesn't always save.**
  ...
  and add no signal). The candidate is promoted (and only then does `data/Networks/Episode_N.keras`/
  ...
  log to `data/logs/gating_log.csv`; training metrics log to `data/logs/training_log.csv`.
- **Raw-MCTS pretraining (`pretrain.py`) is a separate pipeline from self-play, sharing only
  `data/Network.keras` as the handoff point.** `uttt/simulation/raw_mcts.py` is `MCTS(network=None)`
  (random-rollout evaluation, uniform priors over legal moves) — it deliberately skips
  `uttt.worker.configure_cpu_worker()`, since that helper's GPU/thread config calls would force a
  ...
  MCTS iterations per move. `uttt.training.generate_raw_pool.generate_pool()` is idempotent (a
  no-op once the pool reaches `max_training_examples`), which is what lets `pretrain.py` call it
  unconditionally as step one of a single unattended run. `PretrainManager.pretrain()` then trains
  `data/Network.keras` in place each round — no `clone_network()`, no gating, since there's no
  opponent to gate against — and checkpoints to `data/Networks/Pretrain_N.keras` every round rather
  than sparsely. `uttt/evaluation/network_tournament.py`'s checkpoint discovery
  (`discover_checkpoints`/`snapshot_checkpoints`/`build_network_specs`) takes a `prefix` parameter
  so it ladders both `Episode_*` and `Pretrain_*` checkpoints side by side. Like `Episode_N`,
  `Pretrain_N` numbering restarts at 0 every `pretrain.py` run, so re-running it after switching
  pools can silently overwrite the previous run's checkpoint files (`data/Network.keras` itself
  always holds the latest weights regardless).
- **A "fresh" run is often actually a resume.** `project.py` only builds a new `convNet` if
  `data/Network.keras` doesn't exist, and `TrainingManager.__init__` separately loads the
  most-recently-modified file in `data/TrainingExamples/` (by mtime, not the highest `Episode_N` —
  numbering restarts at 0 every run) as the starting replay buffer, keeping it under one fixed
  backup name (`data/TrainingExamples/_resumed_from.pkl`) and deleting every other file. `python
  project.py` is only really starting from scratch if both `data/Network.keras` and
  `data/TrainingExamples/` are absent/empty.
